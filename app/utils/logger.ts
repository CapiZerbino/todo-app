...
const formatMessages = (messages: Array<any>, type?: 'success' | 'info' | 'error' | 'start' | 'warning' | 'end') => {
	let color = 'black';
	let bgc = 'White';
	switch (type) {
		case 'success':
	...
	return [`%c[${type?.toUpperCase()} - @${dayjs(new Date()).format('HH:mm:ss')}] ➟`, `🐦;background: ${bgc}; color: ${color}; padding: 3px; border-radius: 5px;`, ...messages];
};
